...
Y_tensor = torch.tensor(Y, dtype=torch.float32).view(-1, 1)# one column and as many rows as needed
...
def train_and_evaluate(model_class, hidden_nodes, learning_rate):
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch {epoch+1}/{epochs}, Loss: {train_losses[-1]:.6f}')# the average loss of the most recent epoch
    return model, train_losses

def train_and_test_with_batch_size(model_class, batch_size, hidden_nodes, learning_rate):
    # hidden_nodes=32
    model = model_class(hidden_nodes)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
...
for hidden_nodes in hidden_nodes_range:
    model, train_losses = train_and_evaluate(ShallowCorrectorMLP, hidden_nodes, learning_rate=0.00001)
    ...
    plt.title(f'Trajectory Tracking Comparison with {hidden_nodes} Hidden Nodes')
    plt.xlabel('Time [s]')
    plt.ylabel('Position')
    plt.legend()
    ...
    try:
        ...
        print(f"Figure for trajectory with {hidden_nodes} hidden nodes saved successfully")
...
for lr in learning_rates:
    shallow_model, shallow_train_losses = train_and_evaluate(ShallowCorrectorMLP, hidden_nodes, lr)
    deep_model, deep_train_losses = train_and_evaluate(DeepCorrectorMLP, hidden_nodes, lr)
    q_real_shallow = PD_and_MLP_correction(shallow_model, q_target, dot_q_target, t)
...
for batch_size in batch_sizes:
    ...
    shallow_train_losses, shallow_test_loss, shallow_training_time = train_and_test_with_batch_size(ShallowCorrectorMLP, batch_size, hidden_nodes=32, learning_rate=0.00001)
    
    print(f"\nEvaluating DeepCorrectorMLP with batch size {batch_size}")
    deep_train_losses, deep_test_loss, deep_training_time = train_and_test_with_batch_size(DeepCorrectorMLP, batch_size, hidden_nodes=32, learning_rate=0.00001)
...
plt.plot([result['batch_size'] for result in results], [result['shallow_training_time'] for result in results], label='Shallow MLP Training Time')
plt.plot([result['batch_size'] for result in results], [result['deep_training_time'] for result in results], label='Deep MLP Training Time')
...
for result in results:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot([result['shallow_train_losses'] for result in results], label='Shallow MLP Train Losses')
    plt.plot([result['deep_train_losses'] for result in results], label='Deep MLP Train Losses')
    plt.xlabel('Epchos')
    plt.ylabel('Training Loss')
    plt.title(f'Comparison of Training Loss for {result["batch_size"]} Batch Size')
    plt.legend()
    ...
    try:
        plt.savefig(file_path)
        print(f'figure for Training Losses for {result["batch_size"]} Batch Size saved successfully')
    except Exception as e:
        print(f'Error saving figure for Training Losses for {result["batch_size"]} Batch Size: {e}')
    ...
    try:
        plt.savefig(file_path)
        print(f'figure for Test Losses for {result["batch_size"]} Batch Size saved successfully')
    except Exception as e:
        print(f'Error saving figure for Test Losses for {result["batch_size"]} Batch Size: {e}')
